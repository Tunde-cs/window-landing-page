...
)  # Allow GET for debugging, POST for actual use
def chat():
    try:
        if request.method == "GET":
            ...
            return jsonify(
                {"message": "Chat endpoint is running. Use POST to send messages."}
            )
        ...
        return (
            jsonify({"error": "OpenAI API error occurred. Please try again later."}),
            500,
        )
